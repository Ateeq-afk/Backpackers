...
useEffect(() => {
  ...
  // Handler for when an option is selected
    ...
    const handleRemoveArrayItem = (name: keyof ActData, index: number) => {
      ...
      const handleDayChange = (index: number, e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        setActData(prev => {
          const updatedDays = [...prev.days as Day[]]; // Type assertion
      ...
      const handleDescriptionChange = (dayIndex: number, descIndex: number, e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        const newDays = [...actData.days];
        newDays[dayIndex].description[descIndex] = e.target.value;
        setActData({ ...actData, days: newDays });
      };
      
      const addDescription = (dayIndex: number) => {
        const newDays = [...actData.days];
      ...
      const handleSubmit = async (e: React.FormEvent) => {
        ...
        for (const [key, value] of Object.entries(actData)) {
            if (!['days', 'highlight', 'confirmation', 'cancellation','over','related','similar','booking','desc','blogs','faq'].includes(key)) {
        ...
        actData.days.forEach((day, index) => {
          ...
          formData.append('days', JSON.stringify(actData.days));
    ...
    // }
      ...
      if (Array.isArray(actData.related)) {
        ...
        try {
          const response = await fetch('https://launch-api1.vercel.app/activity/createactivity', {
            method: 'POST',
            body: formData,
          });
  ...
  return (
   ...
   <h1 className='text-5xl text-center mb-5 font-bold text-yellow-500'>Create Activity</h1>
...
<div className='w-full px-8 pt-6 pb-8 border border-gray-700 rounded-xl'>
      <h3 className="text-4xl text-center font-semibold text-gray-700 mb-4">Itinerary Section</h3>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        ...
        <button
          ...
          className="px-4 py-2 mt-2 bg-yellow-500 text-white rounded transition-colors duration-300"
        >
          Add Description
        </button>
        </div>
...
<div className='w-full px-8 pt-6 pb-8 border border-gray-700 rounded-xl'>
      <h3 className="text-4xl text-center font-semibold text-gray-700 mb-4">Similar Activities Section</h3>
